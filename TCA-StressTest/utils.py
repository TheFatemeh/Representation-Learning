    ...
    for cls_idx, items in cache.items():
        for i, item in enumerate(items):
            if not args.token_sim:
                ...
                features.append(
                    item[2].squeeze().flatten().cpu().numpy()  # cls_token_list (item[2]); not item[-1], which is now a tag
                )
            indices.append(
                (cls_idx, i)
...
def find_prototypes_with_indices(cache, num_prototypes=3, args=None, clip_model=None):
    ...
    for cls_idx, items in cache.items():
        if len(items) != 0:
            if args.token_sim:
                features.extend([i[2].ravel() for i in items])  # cls_token_list (item[2]); not item[-1], which is now a tag
            else:
