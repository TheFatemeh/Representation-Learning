...
def measure_visual_gflops(clip_model, res=224):
    """One-shot FLOP count of the CLIP visual encoder. TDA does NOT prune tokens, so this equals
    the CLIP baseline (~17.59). Constant per backbone, content-independent."""
    import logging
    from fvcore.nn import FlopCountAnalysis
    logging.getLogger("fvcore.nn.jit_analysis").setLevel(logging.ERROR)
    try:
        w = clip_model.visual.conv1.weight  # match conv1 dtype (model is fp16 on cuda)
...
def main():
    ...
    # TDA keeps all tokens (no pruning) -> visual-encoder GFLOPs == CLIP (~17.59). Constant per backbone.
    ...
    # run_test_tda calls wandb.log() every iteration unconditionally; when --wandb-log is not
    # passed, init a disabled run so those calls are no-ops instead of erroring. (Logging only.)
    if not args.wandb:
